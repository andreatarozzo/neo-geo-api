...
# This is particularly useful in Docker Compose setups, where the database container
# might pass health checks but still be initializing, preventing it from accepting connections
# and so preventing the node application to fail its DB connection check.
max_retries=60
retries=0
...
if [ "$NODE_ENV" = "production" ]; then
  ...
  # meaning that no volume was previously present for the DB.
